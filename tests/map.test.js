...
test.only("our coffee profiles are all available in d3 geo", () => {
  const coffeeCountries = Object.keys(coffeeProfiles);
  const d3Countries1 = topojson.feature(world, world.objects.countries);

  const d3Countries = d3Countries1.features.map(
    (feature) => d3CountryNameNormilizer(feature.properties.name)
  );
